...
专注的 Zotero PDF 逐句翻译插件，支持机器翻译、AI 翻译与一键标注。点击 PDF 中的句子即可对照两种译文，并通过色块一键保存为彩色高亮标注。

## 功能

- **双引擎简译** — 在 PDF 阅读器中点击任意句子，同时查看机器翻译与 AI 翻译；两者可独立折叠，机器引擎和 AI 展开策略可直接在浮窗切换。
- **复用“翻译”插件引擎** — 动态读取 Translate for Zotero 中已配置的句子翻译服务，不限制启用数量，并使用相同的本地化或自定义引擎名称。
- **按需使用 AI** — AI 可保持自动展开，也可默认关闭、展开时才请求；AI 缓存命中时自动展开。详解和解析中的简译优先使用 AI，没有 AI 结果时回退到机器翻译。
...
如需机器翻译，请同时安装并配置 [Translate for Zotero](https://github.com/windingwind/zotero-pdf-translate)。本插件直接复用其中的引擎、密钥与语言方向。
