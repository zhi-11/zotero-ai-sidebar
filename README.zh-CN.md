# 点击翻译句子

[English](README.md) | [中文](README.zh-CN.md)

由 **zhi-11 & Codex** 维护。项目主页：[zhi-11/zotero-ai-sidebar](https://github.com/zhi-11/zotero-ai-sidebar)。

专注的 Zotero PDF 逐句翻译插件，支持机器翻译、AI 翻译与一键标注。点击 PDF 中的句子即可对照两种译文，并通过色块一键保存为彩色高亮标注。

## 功能

- **双引擎简译** — 在 PDF 阅读器中点击任意句子，同时查看机器翻译与 AI 翻译；两者可独立折叠，机器引擎和 AI 展开策略可直接在浮窗切换。
- **机器翻译小窗口** — 机器译文只在内存中保留上一句、当前句和下一句，并按“当前 → 下一句 → 上一句”的顺序串行准备，方便前后回看。
- **复用“翻译”插件引擎** — 动态读取 Translate for Zotero 中已配置的句子翻译服务，不限制启用数量，并使用相同的本地化或自定义引擎名称。
- **按需使用 AI** — AI 可保持自动展开，也可默认关闭、展开时才请求；AI 缓存命中时自动展开。详解和解析中的简译优先使用 AI，没有 AI 结果时回退到机器翻译。
- **提前预翻译** — AI 保持自动展开时，可自定义领先阅读进度的缓存句数；当前句完成后按顺序逐句翻译，每前进一句就自动补齐窗口末尾。填 0 关闭，AI 默认关闭时也不会预翻译。
- **逐句问答** — “问答”模式默认展示译文和输入框，回答保持简短；同一句可连续追问，并将多组 `Q/A` 追加到已有批注而不重复创建高亮或下划线。
- **模式界面可配** — 简译、详解、解析、问答可以分别显示或隐藏，并可在独立的界面设置中调整排列顺序和默认模式。
- **彩色标注** — 翻译完成后，点击色块即可将句子保存为对应颜色的 PDF 高亮。12 种预设颜色对应常见研究分类（方法、结果、背景、术语等），可在设置中自定义。
- **左右优先定位** — 针对双栏论文，翻译浮窗优先显示在句子旁边（自动模式：右侧 → 左侧 → 下方），不遮挡阅读。
- **智能断句** — 属名缩写（如 `A. japonicus`）自动识别，不会被误断。分类学常用缩写（`sp.` `spp.` `var.` `cf.` `aff.`）可配置排除。
- **自带模型** — 支持 Anthropic、OpenAI 及兼容接口，API Key 本地存储在 Zotero 偏好设置中。
- **字体大小可调** — 翻译框字体 10-28px 自由调节。
- **快捷键可配** — 切换翻译模式的快捷键可自定义，带「记录」按钮，Alt+T 作为后备。

## 安装

1. 从 [GitHub Releases](https://github.com/zhi-11/zotero-ai-sidebar/releases/latest) 下载最新 `.xpi`。
2. 打开 Zotero 7/8/9/10。
3. `工具` → `插件` → 齿轮图标 → `从文件安装插件`。
4. 选择下载的 `.xpi`，按提示重启 Zotero。

## 配置

在 Zotero 插件设置中至少配置一个模型账号：

- Provider：`anthropic` 或 `openai`
- API Key：本地存储
- Base URL：官方或兼容地址
- Model：任意支持的模型 ID

如需机器翻译，请同时安装并配置 [Translate for Zotero](https://github.com/windingwind/zotero-pdf-translate)。本插件直接复用其中的引擎、密钥与语言方向。

翻译设置：

- **触发方式**：单击或双击翻译
- **结果位置**：自动（优先左右）/ 右侧 / 左侧 / 上方 / 下方
- **翻译框大小**：紧凑或自适应
- **上下文**：仅本句 / 本段 / 整页
- **快捷键**：Enter 下一句，Shift+Enter 上一句
- **字体大小**：10-28px（默认 14）
- **翻译模式快捷键**：可自定义，留空仅用 Alt+T
- **断句例外词**：不会被句号断开的词（如 `sp` `spp`）
- **标注颜色**：12 种可自定义颜色预设
- **问答批注**：可选择高亮或下划线、单独设置颜色，并选择回答后自动写入或手动添加
- **悬浮框界面**：调整四种模式的显示状态、顺序和默认模式
- **AI 预翻译**：自动展开时自定义提前缓存的后续句数，设为 0 即关闭

## 开发

```bash
npm install
npm run build
```

构建输出在 `.scaffold/build/`。

## 许可

AGPL-3.0-or-later。

## 作者与上游

本衍生版本由 **zhi-11 & Codex** 维护，基于 [xuhan-rgb/zotero-ai-sidebar](https://github.com/xuhan-rgb/zotero-ai-sidebar) 开发；原项目及其贡献者继续按照 AGPL-3.0-or-later 许可证保留署名。
